...
// so this just renders whatever's handed to it. Sentiment on a response that
// never mentions the brand isn't meaningful, so non-mentions are excluded here
// rather than relying on every caller to pre-filter.
export function SentimentBreakdown({
  results,
  title = 'Sentiment Breakdown',
}: {
  results: { sentiment: string; isMentioned: boolean }[]
  title?: string
}) {
